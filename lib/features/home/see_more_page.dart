...
class _SeeMorePageState extends ConsumerState<SeeMorePage> {
  ...
  bool _lastIsNsfw = false;

  @override
  void initState() {
    super.initState();
    // Safely trigger the Riverpod read after the first frame is built
    Future.microtask(() {
      _lastIsNsfw = ref.read(settingsProvider).showAdultContent;
      _fetchPage(1);
    });
  }
  ...
  Widget build(BuildContext context) {
    ...
    ref.listen<SettingsState>(settingsProvider, (previous, next) {
      if (previous?.showAdultContent != next.showAdultContent) {
        // Setting changed while on page -> invalidate cache and reload
        _lastIsNsfw = next.showAdultContent;
        _pageCache.clear();
        _fetchPage(1);
      }
    });
