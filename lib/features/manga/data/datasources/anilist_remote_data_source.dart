...
class AnilistRemoteDataSource {
  ...
  static const int _cacheDuration = 10800000;
  static const Duration _timeoutDuration = Duration(seconds: 20);

  DateTime _nextAllowedRequestTime = DateTime.now();
  final Duration _minRequestInterval = const Duration(milliseconds: 800);

  Future<void> cleanCache() async {
    await LocalCacheService.cleanAniListCache(_cacheDuration);
  }

  Future<void> invalidateSpecificCaches(List<String> cacheKeys) async {
      ...
      try {
        ...
        if (response.statusCode == 429) {
          ...
          if (response.headers.containsKey('retry-after')) {
            ...
            final resetTimeEpoch = int.tryParse(
          ...
          _nextAllowedRequestTime = DateTime.now().add(
            Duration(seconds: waitSeconds + 1),
          );
  ...
  Future<List<dynamic>> fetchStandardList({
    required String query,
    ...
    try {
      final Map<String, dynamic> finalVariables = variables != null
          ? Map.from(variables)
          : {};
  ...
  Future<Map<String, dynamic>?> getPersonDetails(int id, bool isStaff) async {
  ...
  Future<Map<String, dynamic>?> getFullPersonList({
  ...
  Future<PaginatedResult?> fetchPaginatedManga({
    required int page,
    required bool isNsfw,
    ...
    final Map<String, dynamic> variables = {'page': page, 'sort': sort};
    ...
    try {
      final response = await _performRequest(
        query: AniListQueries.queryPaginatedManga,
        variables: variables,
      );
  ...
  Future<PaginatedResult?> fetchPaginatedRecommendations({
  ...
  Future<Map<String, List<String>>> fetchAvailableFilters({
    ...
    final cacheKey = isNsfw
        ? 'anilist_filters_list_nsfw'
        : 'anilist_filters_list_sfw';
    const int filtersCacheDuration = 86400000; // 24 hours

    final cachedData = await LocalCacheService.getAniListCache(
      cacheKey,
      filtersCacheDuration,
    );
    if (cachedData != null) {
      return {
    ...
    try {
      final response = await _performRequest(
        query: AniListQueries.getGenresAndTags,
      );
      ...
      if (response.statusCode == 200) {
        ...
        List<String> genres = List<String>.from(data['GenreCollection']);
        final List<dynamic> rawTags = data['MediaTagCollection'];

        if (!isNsfw) {
          genres = genres.where((g) => g != 'Hentai').toList();
        }

        final List<String> tags = rawTags
            .where((t) {
              if (!isNsfw && t['isAdult'] == true) return false;
              return true;
            })
    ...
    if (isNsfw && !defaultGenres.contains('Hentai')) {
      defaultGenres.add('Hentai');
      defaultGenres.sort();
    }
