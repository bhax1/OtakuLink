...
import 'package:otakulink/features/profile/profile_widgets/manga_search_delegate.dart';
...
class _EditTopPicksPageState extends ConsumerState<EditTopPicksPage> {
  late List<TopPickEntity?> _slots;
  ...
  void initState() {
    super.initState();
    _slots = List<TopPickEntity?>.filled(5, null);
    _loadInitialData();
  }
  ...
  Future<void> _loadInitialData() async {
    try {
      ...
      SecureLogger.logError("EditTopPicksPage _loadInitialData", e, stack);
      if (mounted) {
        setState(() => _isLoading = false);
        AppSnackBar.show(
          context,
          'Error loading picks: $e',
  ...
  Future<void> _save() async {
    ...
    try {
      ...
      SecureLogger.logError("EditTopPicksPage _save", e, stack);
      if (mounted) {
  ...
  void _openSearch(int index) async {
    ...
    final bool isNsfw = settings.showAdultContent;
    const bool isDataSaver = false; // Add this to settings later if needed
