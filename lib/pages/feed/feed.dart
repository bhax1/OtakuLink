  ...
  Future<void> _fetchFriends() async {
    ...
    if (userDoc.exists) {
      ...
      if (data != null && data['friends'] != null) {
        if (mounted) setState(() => _myFriendsIds = List<String>.from(data['friends']));
      }
    }
  }
          ...
          slivers: [
            const SliverToBoxAdapter(
              child: Padding(
                ...
                child: Text("Social Feed", style: TextStyle(fontSize: 24, fontWeight: FontWeight.bold)),
              ),
            ),
            const SliverToBoxAdapter(child: NewsRail()),
            const SliverToBoxAdapter(child: Divider(height: 24, thickness: 1)),
            FeedStreamSliver(friendIds: _myFriendsIds, onTabChange: widget.onTabChange),
          ],
        ),
      ),
...
class FeedStreamSliver extends StatelessWidget {
  ...
  const FeedStreamSliver({super.key, required this.friendIds, required this.onTabChange});
  ...
  Widget build(BuildContext context) {
    return StreamBuilder<QuerySnapshot>(
      ...
      builder: (context, snapshot) {
        ...
        if (docs.isEmpty) {
          return const SliverToBoxAdapter(child: Center(child: Padding(padding: EdgeInsets.all(32), child: Text("No posts yet."))));
        }
...
class _CreatePostDialogState extends State<CreatePostDialog> with SingleTickerProviderStateMixin {
  ...
  final List<String> _activities = ['Reading', 'Watching', 'Playing', 'Listening to', 'Feeling'];
  ...
  Future<void> _submitPost() async {
    if (_textController.text.trim().isEmpty && _tabController.index != 1 && _linkedManga == null) return;
    ...
    if (typeIndex == 1) { // POLL
      ...
      if (_isImagePoll) {
        ...
        List<String> options = _pollOptions.map((c) => c.text.trim()).where((t) => t.isNotEmpty).toList();
        if (options.length < 2) return;
        extraData['pollOptions'] = options;
      }
      extraData['pollVotes'] = {};
    } else if (typeIndex == 2) { // Q&A
      type = 'qa';
    } else if (typeIndex == 3) { // ACTIVITY
      type = 'activity';
                ...
                child: SizedBox(
                  ...
                  child: TabBarView(
                    ...
                    children: [
                      ...
                      _buildWithLinkButton(_buildTextInput("Ask something...", bgColor: Colors.red.withOpacity(0.05))),
    ...
    return SingleChildScrollView(
      ...
      child: Column(
        children: [
          ...
          TextField(
            controller: _textController,
            decoration: const InputDecoration(hintText: "Ask a question...", border: UnderlineInputBorder()),
          ),
          const SizedBox(height: 16),
          ...
          if (!_isImagePoll) ...[
              ...
              return Padding(
                ...
                child: TextField(
                  ...
                  decoration: InputDecoration(
                    ...
                    prefixIcon: const Icon(Icons.radio_button_unchecked, size: 16),
            ...
            TextButton.icon(
              onPressed: () {
                if (_pollOptions.length < 9) setState(() => _pollOptions.add(TextEditingController()));
              },
            ...
            if (_imageOptions.isEmpty) 
              const Text("Add at least 2 characters", style: TextStyle(color: Colors.grey)),
            ...
            Wrap(
              ...
              children: List.generate(_imageOptions.length, (index) {
                ...
                return Chip(
                  avatar: CircleAvatar(backgroundImage: NetworkImage(opt['image']!)),
                  label: Text(opt['text']!),
                  onDeleted: () => setState(() => _imageOptions.removeAt(index)),
                );
              }),
            ),
            ...
            if (_imageOptions.length < 9) 
              OutlinedButton.icon(
                onPressed: () async {
                  final linkedId = _linkedManga != null ? _linkedManga!['id'] : null;
      ...
      child: Column(
        children: [
          DropdownButtonFormField<String>(
            ...
            items: _activities.map((String val) => DropdownMenuItem(value: val, child: Text(val))).toList(),
  ...
  Future<void> _performSearch(String query) async {
    ...
    try {
      final response = await http.post(
        ...
        body: json.encode({'query': graphQLQuery, 'variables': {'search': query}}),
      );
            ...
            : _results.isEmpty 
                ...
                : ListView.separated(
                    ...
                    itemBuilder: (context, index) {
                      ...
                      return ListTile(
                        leading: Image.network(image, width: 40, fit: BoxFit.cover),
